...
function createAnalysisPrompt(payload = {}) {
  ...
  return [
    ...
    JSON.stringify({
      ...
      evidence: ["근거 1", "근거 2"],
      inspectFirst: "가장 먼저 확인할 파일/함수/API/파라미터",
      debugSteps: ["확인 절차 1", "확인 절차 2", "확인 절차 3"],
      fixSuggestion: "수정 방향",
...
function buildFallbackAnalysis(rawText, providerLabel) {
  return {
    summary: `${providerLabel} 응답을 구조화하지 못했습니다.`,
    rootCause: rawText || "응답 텍스트가 비어 있습니다.",
    evidence: [],
    inspectFirst: "",
    debugSteps: [],
    fixSuggestion: "",
  ...
  try {
    ...
    if (!analysis) {
      return {
        ...
        provider: "openai",
        model,
        analysis: buildFallbackAnalysis(rawText, "OpenAI"),
        rawText,
      };
    }
